module Form = {
  ...
  let make = (~userId, ~loading, ~addPost) => {
    let (data: PostForm.data, setData) = React.useState(() => {PostForm.title: "", content: ""})
...
let make = () => {
  ...
  let addPost = (authorId, title, content) =>
    mutate({authorId: authorId, title: title, content: content})->ignore
  let (loading, setLoading) = React.useState(() => false)
  let user = UserContext.use()

  React.useEffect1(() => {
    let loading = switch mutationResult {
    | {data: Some({addPost})} =>
      {
        open Route
        Post(addPost.id, None)->navigate
      }
      false
    | {loading: true} => true
    | _ => false
