module Comments = {
  ...
  let make = (~postId, ~focusedCommentId=?) => {
    ...
    | {data: Some({post: Some({comments})})} =>
      <CommentList comments={comments->Belt.Array.map(c => (c.id, c))} ?focusedCommentId />
    | _ => React.null
    }
  }
}
...
let make = (~postId, ~focusedCommentId) => {
  ...
  let (commentFormVisible, setCommentFormVisible) = React.useState(() => false)
  ...
  | {data: Some({post: maybePost})} =>
    ...
    | Some(post) => <>
        <PostDisplay post />
        <Button className="mb-3" onClick=toggleCommentFormVisible style=Button.Info icon="comment">
