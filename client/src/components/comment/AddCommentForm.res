module Form = {
  ...
  let make = (~userId, ~postId, ~loading, ~addComment) => {
    ...
    <form>
      ...
      <FormField>
        <Button onClick=onSave enabled={contentValid && !loading} icon="save" loading>
          {"Save"->React.string}
        </Button>
      </FormField>
    </form>
...
let make = (~postId) => {
  ...
  let addComment = (authorId, postId, content) =>
    mutate(
      ~refetchQueries=[
        {
          open CommentQueries.PostCommentsQuery
          refetchQueryDescription(makeVariables(~postId, ()))
        },
      ],
      {authorId: authorId, postId: postId, content: content},
    )->ignore
