...
let make = (~userSelect) => {
  ...
  let burgerClass = "navbar navbar-burger has-background-light" ++ (expanded ? " is-active" : "")
  ...
  <nav className="navbar is-fixed-top has-background-light" role="navigation">
    ...
    <div className=navbarMenuClass>
      <div className="navbar-start">
        <RequireAuthor> {linkItem(Route.AddPost, "edit", "New post"->React.string)} </RequireAuthor>
        {linkItem(Route.Authors, "users", "Authors"->React.string)}
        {switch user {
        | None => React.null
        | Some({id}) => linkItem(Route.User(id), "user", "My profile"->React.string)
        }}
      </div>
    </div>
  </nav>
